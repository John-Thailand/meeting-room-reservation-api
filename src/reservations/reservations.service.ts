...
export class ReservationsService {
  ...
  async updateReservation(
    ...
    // その予約が存在しない場合はエラーを返す
    const existingReservation = await this.repo.findOneBy({
      id,
      is_deleted: false,
    })
    ...
    const userId = attrs.user_id ?? existingReservation.user_id
    const meetingRoomId = attrs.meeting_room_id ?? existingReservation.meeting_room_id
    const startDatetime = attrs.start_datetime ?? existingReservation.start_datetime
    const endDatetime = attrs.end_datetime ?? existingReservation.end_datetime
    ...
    Object.assign(existingReservation, attrs)
    return this.repo.save(existingReservation)
  }
